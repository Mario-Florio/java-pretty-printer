package com.marioflorio.prettyprinter.core.usecases;
...
public class StringRenderer implements Renderer {
  ...
  public String renderWrapper(Wrapper wrapper, int indentLevel) {
    switch (wrapper.type()) {
      ...
      case FG_COLOR_GREEN:
        if (applyColor == true) return fgGreenWrapper(wrapper.doc().render(this, indentLevel));

      case FG_COLOR_YELLOW:
        if (applyColor == true) return fgYellowWrapper(wrapper.doc().render(this, indentLevel));

      case FG_COLOR_CYAN:
        if (applyColor == true) return fgCyanWrapper(wrapper.doc().render(this, indentLevel));
  ...
  public String renderConcat(Concat concat, int indentLevel) {
    ...
    for (Doc child : concat.children()) sb.append(child.render(this, indentLevel));
